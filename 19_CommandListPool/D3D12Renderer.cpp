...
lb_exit:
	...
	for (DWORD i = 0; i < MAX_PENDING_FRAME_COUNT; i++)
	{
		...
		m_ppCommandListPool[i]->Initialize(m_pD3DDevice, D3D12_COMMAND_LIST_TYPE_DIRECT, 256);
...
{
	CCommandListPool*	pCommandListPool = m_ppCommandListPool[m_dwCurContextIndex];	// 현재 사용중인 command list pool
...
void CD3D12Renderer::Cleanup()
{
	...
	for (DWORD i = 0; i < MAX_PENDING_FRAME_COUNT; i++)
	{
		if (m_ppCommandListPool[i])
		{
			delete m_ppCommandListPool[i];
			m_ppCommandListPool[i] = nullptr;
		}
