...
BOOL __stdcall CBasicMeshObject::BeginCreateMesh(const BasicVertex* pVertexList, DWORD dwVertexNum, DWORD dwTriGroupCount)
{
	...
	BOOL bUseGpuUploadHepas = m_pRenderer->INL_IsGpuUploadHeapsEnabled();

	if (dwTriGroupCount > MAX_TRI_GROUP_COUNT_PER_OBJ)
		__debugbreak();

	if (FAILED(pResourceManager->CreateVertexBuffer(sizeof(BasicVertex), dwVertexNum, &m_VertexBufferView, &m_pVertexBuffer, (void*)pVertexList, bUseGpuUploadHepas)))
	{
		__debugbreak();
		goto lb_return;
	}
...
BOOL __stdcall CBasicMeshObject::InsertTriGroup(const WORD* pIndexList, DWORD dwTriCount, const WCHAR* wchTexFileName)
{
	...
	BOOL bUseGpuUploadHeaps = m_pRenderer->INL_IsGpuUploadHeapsEnabled();
	...
	if (FAILED(pResourceManager->CreateIndexBuffer(dwTriCount * 3, &IndexBufferView, &pIndexBuffer, (void*)pIndexList, bUseGpuUploadHeaps)))
	{
		__debugbreak();
		goto lb_return;
	}
