...
HRESULT CD3D12ResourceManager::CreateVertexBuffer(UINT SizePerVertex, DWORD dwVertexNum, D3D12_VERTEX_BUFFER_VIEW* pOutVertexBufferView, ID3D12Resource **ppOutBuffer, void* pInitData, BOOL bUseGpuUploadHeaps)
{
	HRESULT hr = S_OK;
...
HRESULT CD3D12ResourceManager::CreateIndexBuffer(DWORD dwIndexNum, D3D12_INDEX_BUFFER_VIEW* pOutIndexBufferView, ID3D12Resource **ppOutBuffer, void* pInitData, BOOL bUseGpuUploadHeaps)
{
	HRESULT hr = S_OK;
	...
	if (pInitData)
	{
		if (bUseGpuUploadHeaps)
		{
			...
			hr = pIndexBuffer->Map(0, &writeRange, reinterpret_cast<void**>(&pIndexDataBegin));
			if (FAILED(hr))
			{
				__debugbreak();
				goto lb_return;
...
BOOL CD3D12ResourceManager::CreateTextureFromFile(ID3D12Resource** ppOutResource, D3D12_RESOURCE_DESC* pOutDesc, const WCHAR* wchFileName, BOOL bUseGpuUploadHeaps)
{
	BOOL bResult = FALSE;
	...
	if (bUseGpuUploadHeaps)
	{
		...
		{
			pTexResource->WriteToSubresource(i, nullptr, subresouceData[i].pData, subresouceData[i].RowPitch, subresouceData[i].SlicePitch);
		}
	}
	else
	{
