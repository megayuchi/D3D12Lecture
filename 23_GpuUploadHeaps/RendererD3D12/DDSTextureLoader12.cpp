...
#undef ISBITMASK
    ...
    HRESULT CreateTextureResource(
        ...
        _Outptr_ ID3D12Resource** ppTexture,
    ...
    ) noexcept
    {
        ...
        hr = d3dDevice->CreateCommittedResource(
            ...
            nullptr,
            IID_ID3D12Resource, reinterpret_cast<void**>(ppTexture));
        if (SUCCEEDED(hr))
        {
        ...
        _Out_opt_ bool* outIsCubeMap) noexcept(false)
    {
        ...
        if (SUCCEEDED(hr))
        {
			...
			hr = CreateTextureResource(d3dDevice, resDim, twidth, theight, tdepth, reservedMips - skipMip, arraySize, format, resFlags, loadFlags, ppTexture, pHeapProp);
            ...
            if (FAILED(hr) && !maxsize && (mipCount > 1))
            {
                ...
                if (SUCCEEDED(hr))
                {
					hr = CreateTextureResource(d3dDevice, resDim, twidth, theight, tdepth, mipCount - skipMip, arraySize, format, resFlags, loadFlags, ppTexture, pHeapProp);
                }
            }
        }
	...
	bool* isCubeMap)
{
    ...
    if (!d3dDevice || !ddsData || !ppTexture)
    {
        return E_INVALIDARG;
    }
	...
	hr = CreateTextureFromDDS(d3dDevice, header, bitData, bitSize, maxsize, resFlags, loadFlags, ppTexture, pHeapProp, subresources, isCubeMap);
    if (SUCCEEDED(hr))
    {
        SetDebugObjectName(*ppTexture, L"DDSTextureLoader");
    ...
    bool* isCubeMap)
{
    ...
    if (!d3dDevice || !fileName || !ppTexture)
    {
        return E_INVALIDARG;
    }
    ...
    if (SUCCEEDED(hr))
    {
        SetDebugTextureInfo(fileName, *ppTexture);
